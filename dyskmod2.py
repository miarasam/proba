...
for x in range(100):
    ...
    wektmod.pro=[c,c1,r[0]]
    a.append(wektmod.pro[0])
    b.append(wektmod.pro[1])
    list.append(wektmod.pro)    #lista z wszytkimi danymi dysku i wszystkich dysków
print(list)
for zx in range(100):
    for z in range(zx+1,100):
        wektmod.kolizja(list[zx],list[z])    #sprawdzenie czy wystepuje kolizja
        for p12 in range(z):
            while wektmod.kolizja(list[p12], list[z]) == True:       #jesli wystepuje przesuniecie dysku
